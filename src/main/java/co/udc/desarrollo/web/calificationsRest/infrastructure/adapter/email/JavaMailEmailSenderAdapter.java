...
public class JavaMailEmailSenderAdapter implements EmailSenderPort {
    ...
    public void send(final EmailDestinationModel destination) {
        try {
            ...
            throw EmailSenderException.becauseSmtpFailed(
                    destination.getDestinationEmail().value(), exception);
        }
    }

    MimeMessage buildMessage(final EmailDestinationModel destination)
        ...
        message.setText(destination.getBody().value(), CHARSET_UTF8, SUBTYPE_HTML);
        return message;
    }
    ...
    private static Properties buildSmtpProperties(final SmtpConfig config) {
        ...
        properties.put(MAIL_SMTP_SSL_TRUST, config.host());
        properties.put(MAIL_SMTP_CONNECTION_TIMEOUT, SMTP_TIMEOUT_MS);
        properties.put(MAIL_SMTP_TIMEOUT, SMTP_TIMEOUT_MS);
        return properties;
    }
